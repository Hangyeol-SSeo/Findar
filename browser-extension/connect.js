...
window.addEventListener("message", async (event) => {
  ...
  try {
    ...
    window.postMessage({ type: "FINDAR_CONNECTED", seq: event.data.seq, ...result }, location.origin);
  } catch { window.postMessage({ type: "FINDAR_CONNECTED", seq: event.data.seq, error: "확장 기능에 연결하지 못했습니다." }, location.origin); }
});
